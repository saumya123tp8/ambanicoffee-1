...
export const confirmProductController = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: true,
      message: "order is delivered or canceled succesfully",
    });
  } catch (error) {
    console.log(error);
    res.status(500).send({
      success: false,
      error,
      message: "error while confirming a product",
    });
  }
};
...
export const productFiltersController = async (req, res) => {
  try {
    ...
    res.status(200).send({
      success: false,
      error,
      message: "error while filtring a product",
    });
  }
};
...
export const productCountController = async (req, res) => {
  try {
    ...
    res.status(400).send({
      success: false,
      error,
      message: "error in count pagination",
    });
  }
};
...
//             buyer: req.user._id,
//           }).save();
//           relatedProductController.json({ ok: true });
//         } else {
      ...
      function (error, result) {
        if (result) {
          ...
          const updatedUser =  userModel.findByIdAndUpdate(
                req.user._id,
                {
                  totalSpent : user.totalSpent+total||total,
                  totalOrder : user.totalOrder+1||1
                },
                { new: true }
              );
          res.json({ ok: true });
          console.log("crreated Order :" + order);
        } else {
          res.status(500).send(error);
        }
      }
